...
import { t as require_docxtemplater } from "./docxtemplater.js";
//#region node_modules/docxtemplater-image-module-free/js/templates.js
...
//#endregion
//#region node_modules/xmldom/sax.js
var require_sax = /* @__PURE__ */ __commonJSMin(((exports) => {
			...
			try {
				...
				switch (source.charAt(tagStart + 1)) {
					...
					default:
						...
						if (el.uri === "http://www.w3.org/1999/xhtml" && !el.closed) end = parseHtmlSpecialContent(source, end, el.tagName, entityReplacer, domBuilder);
						else end++;
				}
			} catch (e) {
		...
		while (true) {
			...
			switch (c) {
					...
					switch (s) {
						...
						case S_ATTR_SPACE:
							...
							if (s == S_ATTR_NOQUOT_VALUE) {
								...
								if (currentNSMap[""] !== "http://www.w3.org/1999/xhtml" || !value.match(/^(?:disabled|checked|selected)$/i)) errorHandler.warning("attribute \"" + value + "\" missed value!! \"" + value + "\" instead!!");
				...
				default: if (c <= " ") switch (s) {
					...
					case S_ATTR_SPACE:
						el.tagName;
						if (currentNSMap[""] !== "http://www.w3.org/1999/xhtml" || !attrName.match(/^(?:disabled|checked|selected)$/i)) errorHandler.warning("attribute \"" + attrName + "\" missed value!! \"" + attrName + "\" instead2!!");
	...
	*/
	function appendElement(el, domBuilder, currentNSMap) {
		...
		while (i--) {
			...
			var prefix = a.prefix;
			if (prefix) {
				if (prefix === "xml") a.uri = "http://www.w3.org/XML/1998/namespace";
	...
	*/
	...
	if (!(_set_proto_({}, _set_proto_.prototype) instanceof _set_proto_)) _set_proto_ = function(thiz, parent) {
		function p() {}
		p.prototype = parent;
		p = new p();
...
//#endregion
//#region node_modules/xmldom/dom.js
var require_dom = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	function _extends(Class, Super) {
		...
		if (pt.constructor != Class) {
			if (typeof Class != "function") console.error("unknow Class:" + Class);
	...
	*/
	...
	function _removeNamedNode(el, list, attr) {
		...
		} else throw DOMException(NOT_FOUND_ERR, /* @__PURE__ */ new Error(el.tagName + "@" + attr));
	}
	...
	*/
	function DOMImplementation(features) {
		this._features = {};
		if (features) for (var feature in features) this._features = features[feature];
	}
	DOMImplementation.prototype = {
		hasFeature: function(feature, version) {
			var versions = this._features[feature.toLowerCase()];
			if (versions && (!version || version in versions)) return true;
			else return false;
		},
	...
	*/
	...
	function _onAddAttribute(doc, el, newAttr) {
		doc && doc._inc++;
		if (newAttr.namespaceURI == "http://www.w3.org/2000/xmlns/") el._nsMap[newAttr.prefix ? newAttr.localName : ""] = newAttr.value;
	}
	function _onRemoveAttribute(doc, el, newAttr, remove) {
		doc && doc._inc++;
		if (newAttr.namespaceURI == "http://www.w3.org/2000/xmlns/") delete el._nsMap[newAttr.prefix ? newAttr.localName : ""];
	}
	...
	function serializeToString(node, buf, isHTML, nodeFilter, visibleNamespaces) {
		...
		switch (node.nodeType) {
			case ELEMENT_NODE:
				...
				if (child || isHTML && !/^(?:meta|link|img|br|hr|input)$/i.test(nodeName)) {
					...
					if (isHTML && /^script$/i.test(nodeName)) while (child) {
						...
						else serializeToString(child, buf, isHTML, nodeFilter, visibleNamespaces);
						child = child.nextSibling;
					}
					else while (child) {
			...
			case DOCUMENT_FRAGMENT_NODE:
				...
				return;
			case ATTRIBUTE_NODE: return buf.push(" ", node.name, "=\"", node.value.replace(/[<&"]/g, _xmlEncoder), "\"");
			case TEXT_NODE: return buf.push(node.data.replace(/[<&]/g, _xmlEncoder));
...
//#endregion
//#region node_modules/xmldom/dom-parser.js
var require_dom_parser = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	DOMParser.prototype.parseFromString = function(source, mimeType) {
		...
		if (source) sax.parse(source, defaultNSMap, entityMap);
...
//#endregion
//#region node_modules/docxtemplater-image-module-free/js/docUtils.js
var require_docUtils = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/docxtemplater-image-module-free/js/imgManager.js
var require_imgManager = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var rels = {
		...
		getRelsFilePath: function getRelsFilePath(fileName, fileType) {
			...
			return (fileType === "pptx" ? "ppt/slides" : "word") + "/_rels/" + relsFileName;
		}
	};
		...
		_createClass(ImgManager, [
			{
				...
				value: function createEmptyRelsDoc(xmlDocuments, relsFileName) {
					...
					if (!doc) {
						var err = /* @__PURE__ */ new Error("Could not copy from empty relsdoc");
						err.properties = {
							mainRels,
							relsFileName,
					...
					for (var i = 0, l = relationshipChilds.length; i < l; i++) relationships.removeChild(relationshipChilds[i]);
			...
			},
			{
				...
				value: function loadImageRels() {
					...
					return Array.prototype.reduce.call(iterable, function(max, relationship) {
						...
						if (/^rId[0-9]+$/.test(id)) return Math.max(max, parseInt(id.substr(3), 10));
						return max;
					}, 0);
				}
			},
			{
				...
				value: function addExtensionRels(contentType, extension) {
					...
					if (Array.prototype.some.call(defaultTags, function(tag) {
			...
			},
			{
				...
				value: function addImageRels(imageName, imageData, i) {
					...
					if (this.zip.files[imagePath] != null) return this.addImageRels(imageName, imageData, i + 1);
					var image = {
						name: imagePath,
						data: imageData,
						options: { binary: true }
...
//#endregion
//#region node_modules/docxtemplater-image-module-free/js/index.js
var require_js = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function getInnerPptx(_ref2) {
		var part = _ref2.part, left = _ref2.left, right = _ref2.right;
		var xmlString = _ref2.postparsed.slice(left + 1, right).reduce(function(concat, item) {
		...
		_createClass(ImageModule, [
			...
			},
			{
				...
				value: function render(part, options) {
					if (!part.type === "placeholder" || part.module !== moduleName) return null;
					var tagValue = options.scopeManager.getValue(part.value, { part });
					if (!tagValue) return { value: this.fileTypeConfig.tagTextXml };
					else if ((typeof tagValue === "undefined" ? "undefined" : _typeof(tagValue)) === "object") return this.getRenderedPart(part, tagValue.rId, tagValue.sizePixel);
			...
			},
			{
				...
				value: function resolve(part, options) {
					...
					return new Promise(function(resolve) {
						resolve(_this.options.getImage(value, part.value));
					}).then(function(imgBuffer) {
						var rId = imgManager.addImageRels(_this.getNextImageName(), imgBuffer);
						return new Promise(function(resolve) {
							resolve(_this.options.getSize(imgBuffer, value, part.value));
			...
			},
			{
				...
				value: function getRenderedPart(part, rId, sizePixel) {
					if (isNaN(rId)) throw new Error("rId is NaN, aborting");
					...
					var newText = void 0;
					if (this.fileType === "pptx") newText = this.getRenderedPartPptx(part, rId, size, centered);
					else newText = this.getRenderedPartDocx(rId, size, centered);
